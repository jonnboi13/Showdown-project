...
def parse_match(match_id: str) -> pl.DataFrame:
    url = f"https://replay.pokemonshowdown.com/{match_id}.json"

    response = requests.get(url)
    if response.status_code != 200:
            raise ValueError(f"Failed to fetch match {match_id}. Status code: {response.status_code}")
    ...
    upload_time = battle_data.get("uploadtime", 0)
    ...
    match_info = grouped_pokemon.with_columns(
        ...
        pl.lit(match_rating if isinstance(match_rating, int) else None).cast(pl.Int32).alias("rating"),
        pl.lit(match_id).alias("match_id"),
        pl.lit(upload_time).alias("uploadtime")
    ).select([
...
my_match_df = parse_match("gen9ou-2668447365")
print(my_match_df)